(function () {
  ...
  function anyOverlayOpen(exceptId) {
    var ids = ["modal", "cartDrawer", "authModal"];
    return ids.some(function (id) {
      if (id === exceptId) return false;
      var el = byId(id);
      return el.classList.contains("is-visible") || el.classList.contains("is-open");
    });
  }

  function closeModal() {
    byId("modal").classList.remove("is-visible");
    if (!anyOverlayOpen("modal")) {
  ...
  function bindEvents() {
    ...
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape") { closeModal(); closeCart(); closeAuth(); }
    });
  ...
  /* ---------------- ACCOUNTS (browser-local only — see note in UI) ---------------- */
  var USERS_KEY = "rann_users_v1";
  var SESSION_KEY = "rann_session_v1";
  var storageOk = true;
  try {
    window.localStorage.setItem("__rann_test__", "1");
    window.localStorage.removeItem("__rann_test__");
  } catch (e) {
    storageOk = false;
  }
  ...
  function setSession(email) {
    if (!storageOk) return;
    try { window.localStorage.setItem(SESSION_KEY, JSON.stringify({ email: email })); } catch (e) { /* ignore */ }
  }
  ...
  function openAuth() {
    byId("authModal").classList.add("is-visible");
    byId("backdrop").classList.add("is-visible");
    document.body.style.overflow = "hidden";
  }
  function closeAuth() {
    byId("authModal").classList.remove("is-visible");
    if (!anyOverlayOpen("authModal")) {
      byId("backdrop").classList.remove("is-visible");
      document.body.style.overflow = "";
    }
  }
  ...
  function renderAuthState() {
    ...
    var loggedOut = byId("authLoggedOut");
    var loggedIn = byId("authLoggedIn");
    var accountBtn = byId("accountToggle");

    if (session) {
      var users = loadUsers();
      var user = users[session.email];
      var name = user ? user.name : session.email;
      loggedOut.style.display = "none";
      loggedIn.style.display = "block";
      byId("authGreeting").textContent = "Hi, " + name.split(" ")[0];
      byId("authEmailDisplay").textContent = session.email;
      byId("authAvatar").textContent = name.charAt(0).toUpperCase();
      accountBtn.setAttribute("aria-label", "Account (" + name + ")");
    } else {
      loggedOut.style.display = "block";
      loggedIn.style.display = "none";
      accountBtn.setAttribute("aria-label", "Log in or sign up");
    }
  }

  function bindAuthEvents() {
    byId("accountToggle").addEventListener("click", function () {
      renderAuthState();
      if (!getSession()) {
        document.querySelectorAll("[data-authtab]").forEach(function (t) {
          t.classList.toggle("is-active", t.dataset.authtab === "login");
        });
        byId("loginForm").style.display = "block";
        byId("signupForm").style.display = "none";
    ...
    if (!storageOk) {
      byId("loginError").textContent = "This browser is blocking local storage, so accounts can't be saved here.";
      byId("signupError").textContent = "This browser is blocking local storage, so accounts can't be saved here.";
    }
    ...
    tabs.forEach(function (tab) {
      tab.addEventListener("click", function () {
        ...
        var which = tab.dataset.authtab;
        byId("loginForm").style.display = which === "login" ? "block" : "none";
        byId("signupForm").style.display = which === "signup" ? "block" : "none";
        byId("loginError").textContent = "";
        byId("signupError").textContent = "";
      });
    });
    ...
    byId("signupForm").addEventListener("submit", function (e) {
      ...
      var errEl = byId("signupError");
      errEl.textContent = "";

      if (!name || !email || password.length < 6) {
        errEl.textContent = "Please fill every field — password needs 6+ characters.";
        return;
      }
      var users = loadUsers();
      if (users[email]) {
    ...
    byId("loginForm").addEventListener("submit", function (e) {
      ...
      if (!user || user.password !== password) {
        errEl.textContent = "Email or password doesn't match any account here.";
        return;
      }
      setSession(email);
      renderAuthState();
